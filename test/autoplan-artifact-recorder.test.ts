...
describe('owned Autoplan pending artifact metadata recorder',()=>{
  ...
  test.each(['C:\\owned', '\\\\server\\share'])('Windows hook transport preserves opaque native identity from %s', nativeRoot=>{
...
function bind(fs,os,path,process){${quote}\n${constructor}\nreturn createAutoplanArtifactRecorder;}`);
      const cwd=path.win32.join(nativeRoot,"repo with ' quote $()"), config=path.win32.join(nativeRoot,'config');
      const stateRoot=path.win32.join(nativeRoot,'state'), qaRoot=path.win32.join(nativeRoot,'qa');
