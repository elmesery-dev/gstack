...
Use Aside's sessions when ready; otherwise import into the fallback. Copying does not prove sign-in.

## 1. Resolve the fallback
...
Navigate to the known target; keep its tab open and unchanged. Both options default off; enable only on explicit request:
...
For examples, profile labels/disambiguation, supported sources and platform setup, read [BROWSER.md's cookie reference](../BROWSER.md#choosing-a-source-and-checking-sign-in).
...
Never request/publish cookie values, passwords, identity/profile text, session details, or raw errors in public logs. Picker labels/counts/status exclude cookie values.
